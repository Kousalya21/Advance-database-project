...
def max_mag():
	...
	print(result_dict)
	while result_dict is not False:
		mag = result_dict['MAG'] if result_dict['MAG'] is not None else 0
		if max_mag < float(mag):
			res = result_dict
			max_mag = float(mag)
		result_dict = ibm_db.fetch_assoc(stmt)
	return res
	...
	sql = "SELECT * from quakes where mag = max(select mag from quakes)"
	stmt = ibm_db.prepare(connection, sql)
	ibm_db.execute(stmt)
	return int(ibm_db.fetch_assoc(stmt))
